...
//      TCPIPServer.world.things must be size(1)
    ...
    "integration" in new After {
      ...
      def makeTcpIpBot(name: String): (GameBot, GameClientObserver) = {
        ...
        val client = system.actorOf(Props(new GameClient(serverAddress, observer)), name = s"gamebot-${name}")
        (
      ...
      val world0 = Await.result((tcpIpServerActor ? "WORLD").mapTo[InMemoryWorld], Duration("5 seconds"))
      ...
      val world1 = Await.result((tcpIpServerActor ? "WORLD").mapTo[InMemoryWorld], Duration("5 seconds"))
      ...
      val world2 = Await.result((tcpIpServerActor ? "WORLD").mapTo[InMemoryWorld], Duration("5 seconds"))
      ...
      val world3 = Await.result((tcpIpServerActor ? "WORLD").mapTo[InMemoryWorld], Duration("5 seconds"))
      world3.things must be size(2)

      observers(0).asInstanceOf[RecordingGameClientObserver].observedMessages must be size(3) // bots(0) joins and then bots(1) joins, then bots(2) joins
      observers(1).asInstanceOf[RecordingGameClientObserver].observedMessages must be size(4) // bots(1) joins and then bots(2) joins, then bots(0) leaves
      observers(2).asInstanceOf[RecordingGameClientObserver].observedMessages must be size(4) // bots(2) joins and then bots(0) leaves
      ...
      println(observers(1).asInstanceOf[RecordingGameClientObserver].observedMessages)
      println(observers(2).asInstanceOf[RecordingGameClientObserver].observedMessages)
