...
class LengthFieldFrameSpec extends Specification with NoTimeConversions {

  sequential

  class TestKitEnvironment(_system: ActorSystem = ActorSystem("LengthFieldFrameSpec"))
    extends TestKit(_system) with Scope {

  }

  "The LengthFieldFrame pipeline" should {
    ...
    "serialize messages from the server and serialize the messages from clients" in new TestKitEnvironment {
      ...
      val originalCmd = ByteString("mumoshu")
